...
export function BankReconciliationModal({
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const addState: Record<string, { selected: boolean; categoryId: number; description: string; tipo: 'entrada' | 'saida' }> = {};
      results.forEach(r => {
        if (r.status === 'ofx_only' && r.ofxTransaction) {
          addState[r.id] = {
            ...
            tipo: r.ofxTransaction.type === 'CREDIT' ? 'entrada' : 'saida',
          };
        }
      });
  ...
  }, [profile, accountFilter, accountId]);
...
interface BankReconciliationRowProps {
  ...
  addState?: { selected: boolean; categoryId: number; description: string; tipo: 'entrada' | 'saida' };
  onToggleAdd: () => void;

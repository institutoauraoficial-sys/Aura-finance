...
interface OFXReconciliationModalProps {
  ...
  invoiceItems: SystemTransaction[];
}
...
export function OFXReconciliationModal({
  ...
  invoiceMonth,
  invoiceItems,
}: OFXReconciliationModalProps) {
  const { t } = useLanguage();
  const { formatCurrency } = useCurrency();
  const { profile } = useUser();
  const { filter: accountFilter } = useAccountFilter();
  const { categories: allCategories } = useCategoriesQuery();
  const categories = allCategories.filter(c => c.tipo === 'saida' || c.tipo === 'ambos');
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Converter invoiceItems para SystemTransaction format
    const systemTxs: SystemTransaction[] = invoiceItems.map(item => ({
      id: item.id,
    ...
    const results = reconcileTransactions(transactions, systemTxs);
    setReconciliation(results);

    // Inicializar estado de itens para adicionar
    const addState: Record<string, { selected: boolean; categoryId: number; description: string }> = {};
    results.forEach(r => {
  ...
  // Ajustar valor do lançamento no sistema para bater com o OFX
  const handleAdjustValue = async (item: ReconciliationItem) => {
    ...
    try {
      ...
      // Atualizar o valor no estado local
      ...
      alert('Erro ao ajustar valor. Tente novamente.');
    } finally {
      setAdjustingItem(null);
    }
  };

  // Excluir lançamento do sistema (system_only)
  ...
  const handleSaveSelected = async () => {
    ...
    try {
      ...
      // Calcular data_prevista baseada no mês da fatura + dia de vencimento
      // Usar o dia da transação OFX como referência
      ...
      for (const [id, item] of Object.entries(itemsToAdd)) {
        ...
        const { error } = await supabase.from('lancamentos_futuros').insert({
          ...
          status: 'pendente',
          cartao_id: cardId,
          data_prevista: ofxDate,
          mes_previsto: invoiceMonth,
          parcelamento: 'false',
          recorrente: false,
        });
      ...
      // Disparar evento para atualizar a fatura
      window.dispatchEvent(new CustomEvent('futureTransactionsChanged'));
    } catch (error) {
      alert('Erro ao salvar lançamentos. Tente novamente.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={step === 'complete' ? 'Conciliação Finalizada' : 'Conciliação de Extrato'}
    ...
    >
      ...
      {step === 'upload' && (
        <div className="space-y-6 py-4">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Importar Extrato</h3>
            <p className="text-sm text-[var(--text-secondary)] max-w-md mx-auto">
              Selecione o arquivo da fatura do cartão <span className="text-[var(--text-primary)] font-medium">{cardName}</span> para conciliar com os lançamentos de <span className="text-[var(--text-primary)] font-medium capitalize">{getMonthLabel()}</span>.
            </p>
          </div>
          ...
          <div className="max-w-sm mx-auto">
            <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-[var(--border-medium)] rounded-xl cursor-pointer hover:border-blue-500/30 hover:bg-blue-500/5 transition-all group">
              ...
              <span className="text-xs text-[var(--text-muted)] mt-1">.ofx, .csv</span>
              <input
                type="file"
                accept=".ofx,.OFX,.csv,.CSV"
          ...
          <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-3 max-w-md mx-auto">
            <p className="text-xs text-amber-400/80 text-center">
              Aceita arquivos <span className="font-medium">OFX</span> (Nubank, Itaú, etc) e <span className="font-medium">CSV</span> (XP, C6, etc). Exporte diretamente do app ou site do seu banco.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div className="bg-[var(--bg-card-inner)] border border-[var(--border-default)] rounded-lg p-3 text-center">
              <p className="text-[10px] sm:text-xs text-[var(--text-tertiary)] mb-1">Total Extrato</p>
            ...
            <div className="bg-green-500/5 border border-green-500/10 rounded-lg p-3 text-center">
              <p className="text-[10px] sm:text-xs text-green-400/70 mb-1">Conciliados</p>
            ...
            <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-3 text-center">
              <p className="text-[10px] sm:text-xs text-amber-400/70 mb-1">Fora do Sistema</p>
            ...
            <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 text-center">
              <p className="text-[10px] sm:text-xs text-red-400/70 mb-1">Sem Extrato</p>
          ...
          <div className="flex items-center gap-1 bg-[var(--input-bg)] rounded-lg p-1">
            {([
              ...
              { key: 'matched', label: 'Conciliados', count: counts.matched },
              { key: 'ofx_only', label: 'Fora do Sistema', count: counts.ofxOnly },
              { key: 'system_only', label: 'Sem Extrato', count: counts.systemOnly },
          ...
          {counts.ofxOnly === 0 && counts.systemOnly === 0 && (
            <div className="flex items-center gap-3 bg-green-500/5 border border-green-500/10 rounded-lg p-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-green-400 font-medium">Extrato 100% conciliado!</p>
                <p className="text-[11px] text-[var(--text-secondary)] mt-0.5">
                  Todos os {counts.matched} lançamentos do extrato batem com o sistema.
                  {adjustedCount > 0 && ` ${adjustedCount} valor(es) ajustado(s).`}
                </p>
              </div>
            </div>
          )}
                ...
                <>
                  {selectedToAddCount > 0 && (
                    <span className="text-xs text-blue-400">
                      {selectedToAddCount} item(ns) para adicionar
                    </span>
                  )}
                  <button
                  ...
                  >
                    ...
                    + Adicionar Selecionados ({selectedToAddCount})
                  </button>
                </>
              ) : (
                <button
      ...
      {step === 'complete' && (
        <div className="space-y-6 py-4 text-center">
          ...
          <div>
            <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Conciliação Finalizada!</h3>
            <p className="text-sm text-[var(--text-secondary)]">
              {savedCount === 0 && adjustedCount === 0 && deletedCount === 0 ? (
                <>
                  Extrato de <span className="text-[var(--text-primary)] font-medium capitalize">{getMonthLabel()}</span> verificado com sucesso. Todos os lançamentos estão corretos.
                </>
              ) : (
                <>
                  {savedCount > 0 && (
                    <><span className="text-blue-400 font-medium">{savedCount}</span> adicionado(s). </>
                  )}
                  {adjustedCount > 0 && (
                    <><span className="text-amber-400 font-medium">{adjustedCount}</span> ajustado(s). </>
                  )}
                  {deletedCount > 0 && (
                    <><span className="text-red-400 font-medium">{deletedCount}</span> excluído(s). </>
                  )}
                  Fatura de <span className="text-[var(--text-primary)] font-medium capitalize">{getMonthLabel()}</span> atualizada.
                </>
              )}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            {savedCount > 0 && (
              <div className="bg-blue-500/5 border border-blue-500/10 rounded-lg p-3 min-w-[100px] text-center">
                <p className="text-[10px] sm:text-xs text-blue-400/70 mb-1">Adicionados</p>
            ...
            {adjustedCount > 0 && (
              <div className="bg-amber-500/5 border border-amber-500/10 rounded-lg p-3 min-w-[100px] text-center">
                <p className="text-[10px] sm:text-xs text-amber-400/70 mb-1">Ajustados</p>
            ...
            {deletedCount > 0 && (
              <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 min-w-[100px] text-center">
                <p className="text-[10px] sm:text-xs text-red-400/70 mb-1">Excluídos</p>
      ...
      {confirmDeleteItem && confirmDeleteItem.systemTransaction && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[var(--bg-card)] border border-[var(--border-medium)] rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl">
            ...
            <h3 className="text-base font-semibold text-[var(--text-primary)] text-center mb-2">Excluir Lançamento?</h3>
            ...
            <div className="bg-red-500/5 border border-red-500/10 rounded-lg p-3 my-4">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-xs font-mono text-red-400 font-medium">
                  {formatCurrency(confirmDeleteItem.systemTransaction.valor)}
                </span>
...
// ==========================================
// Componente de linha de conciliação
// ==========================================
...
interface ReconciliationRowProps {
  ...
  addState?: { selected: boolean; categoryId: number; description: string };
  onToggleAdd: () => void;
...
function ReconciliationRow({
    ...
    return (
      <div className="border rounded-lg bg-zinc-500/5 border-zinc-500/10 opacity-50">
        <div className="p-3 flex items-center gap-3">
          ...
          <span className="text-[10px] text-zinc-600">Excluído</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={cn("border rounded-lg transition-colors", config.bg)}>
      {/* Main Row - use div with onClick instead of button to avoid nested interactive issues */}
      <div
      ...
      >
        <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {item.ofxTransaction && (
                <span className="text-[10px] text-[var(--text-tertiary)]">
                  OFX: {item.ofxTransaction.datePosted.split('-').reverse().join('/')}
                </span>
              )}
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-xs sm:text-sm font-mono font-bold text-[var(--text-primary)]">
              {formatCurrency(item.ofxTransaction?.amount || item.systemTransaction?.valor || 0)}
            </p>
            ...
            {isAdjusted && (
              <p className="text-[10px] text-green-400 font-mono">Ajustado ✓</p>
            )}
          </div>
      ...
      {/* Inline category select for ofx_only items when selected (no expand needed) */}
      {item.status === 'ofx_only' && addState?.selected && !isExpanded && (
        <div className="px-3 pb-3 border-t border-[var(--border-default)]" onClick={(e) => e.stopPropagation()}>
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-[var(--text-tertiary)]">Descrição</label>
              <input
                type="text"
      ...
      {isExpanded && (
        <div className="px-3 pb-3 border-t border-[var(--border-default)]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-3 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="bg-[var(--bg-base)] rounded-lg p-3">
                  <p className="text-[10px] text-[var(--text-tertiary)] uppercase mb-2 font-medium">Extrato (OFX)</p>
                  <p className="text-sm text-[var(--text-primary)]">{item.ofxTransaction?.memo}</p>
                  <p className="text-xs text-[var(--text-secondary)] mt-1">
                    Data: {item.ofxTransaction?.datePosted.split('-').reverse().join('/')}
                  </p>
                  <p className="text-xs text-[var(--text-secondary)]">
                    Valor: {formatCurrency(item.ofxTransaction?.amount || 0)}
                  </p>
                ...
                <div className="bg-[var(--bg-base)] rounded-lg p-3">
                  <p className="text-[10px] text-[var(--text-tertiary)] uppercase mb-2 font-medium">Sistema</p>
                  <p className="text-sm text-[var(--text-primary)]">{item.systemTransaction?.descricao}</p>
                  <p className="text-xs text-[var(--text-secondary)] mt-1">
                    Data: {item.systemTransaction?.data_prevista.split('-').reverse().join('/')}
                  </p>
                  <p className="text-xs text-[var(--text-secondary)]">
                    Valor: {formatCurrency(item.systemTransaction?.valor || 0)}
                  </p>
                  {item.systemTransaction?.categoria_nome && (
                    <p className="text-xs text-[var(--text-secondary)]">
                      Categoria: {item.systemTransaction.categoria_nome}
                    </p>
                  )}
                </div>
              </div>

              {/* Botão ajustar valor quando há diferença */}
              {hasValueDiff && !isAdjusted && (
                <div className="flex items-center justify-between bg-amber-500/5 border border-amber-500/10 rounded-lg p-3">
                  <div>
                    <p className="text-xs text-amber-400 font-medium">Valores diferentes</p>
                    <p className="text-[10px] text-[var(--text-secondary)] mt-0.5">
                      OFX: {formatCurrency(item.ofxTransaction?.amount || 0)} → Sistema: {formatCurrency(item.systemTransaction?.valor || 0)}
                  ...
                  >
                    ...
                    Ajustar para {formatCurrency(item.ofxTransaction?.amount || 0)}
                  </button>
                </div>
              )}
              {isAdjusted && (
                <div className="flex items-center gap-2 bg-green-500/5 border border-green-500/10 rounded-lg p-3">
                  ...
                  <p className="text-xs text-green-400">Valor ajustado com sucesso para {formatCurrency(item.ofxTransaction?.amount || 0)}</p>
                </div>
              )}
            </div>
          )}
          ...
          {item.status === 'ofx_only' && item.ofxTransaction && (
            <div className="mt-3 space-y-3">
              <div className="bg-[var(--bg-base)] rounded-lg p-3">
                <p className="text-[10px] text-[var(--text-tertiary)] uppercase mb-2 font-medium">Detalhes do Extrato</p>
                <p className="text-sm text-[var(--text-primary)]">{item.ofxTransaction.memo}</p>
                <p className="text-xs text-[var(--text-secondary)] mt-1">
                  Data: {item.ofxTransaction.datePosted.split('-').reverse().join('/')} • Valor: {formatCurrency(item.ofxTransaction.amount)}
                </p>
              ...
              {addState?.selected && (
                <div className="bg-blue-500/5 border border-blue-500/10 rounded-lg p-3 space-y-2">
                  <p className="text-[10px] text-blue-400 uppercase font-medium">Adicionar ao Sistema</p>
                  <div>
                    <label className="text-[10px] text-[var(--text-tertiary)]">Descrição</label>
                    <input
                      type="text"
          ...
          {item.status === 'system_only' && item.systemTransaction && (
            <div className="mt-3 space-y-3">
              <div className="bg-[var(--bg-base)] rounded-lg p-3">
                <p className="text-[10px] text-[var(--text-tertiary)] uppercase mb-2 font-medium">Lançamento no Sistema (sem correspondência no extrato)</p>
                <p className="text-sm text-[var(--text-primary)]">{item.systemTransaction.descricao}</p>
                <p className="text-xs text-[var(--text-secondary)] mt-1">
                  Data: {item.systemTransaction.data_prevista.split('-').reverse().join('/')} • Valor: {formatCurrency(item.systemTransaction.valor)}
                </p>
                {item.systemTransaction.categoria_nome && (
                  <p className="text-xs text-[var(--text-secondary)]">Categoria: {item.systemTransaction.categoria_nome}</p>
                )}
                <p className="text-[10px] text-red-400/70 mt-2">
                  Este lançamento existe no sistema mas não foi encontrado no extrato OFX.
                </p>
              </div>

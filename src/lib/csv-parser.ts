...
export function parseCSV(content: string): CSVParseResult {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Ignora pagamentos de fatura (valores negativos grandes geralmente são pagamentos)
    if (originalAmount < 0) {
      skippedRows++;
      continue;
    }
    ...
    // Monta a descrição com info de parcela se disponível
    ...
    transactions.push({
      ...
      originalAmount,
      fitId: generateFitId(date, description, originalAmount, i),
      memo,
    });
  }

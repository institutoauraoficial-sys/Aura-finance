 ...
 */
export async function getUserTokens(supabase: SupabaseClient, usuarioId: number): Promise<(GoogleTokens & { id: number; calendar_id_pf: string; calendar_id_pj: string; sync_enabled_pf: boolean; sync_enabled_pj: boolean }) | null> {
  const { data, error } = await supabase
    .from('google_calendar_integrations')
    .select('id, access_token, refresh_token, expires_at, calendar_id_pf, calendar_id_pj, sync_enabled_pf, sync_enabled_pj')

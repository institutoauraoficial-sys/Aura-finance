...
-- Última Atualização: 09/02/2026 - Feature Bloqueio Imediato (dias_acesso_free = 0)
...
-- Atualizada em 19/01/2026: Adicionada limpeza de histórico de chat do WhatsApp/N8N
...
BEGIN
  ...
  IF v_celular IS NOT NULL AND v_celular != '' THEN
    -- Formato: 5511974178200@s.whatsapp.net
  ...
  -- Deletar transações se solicitado
...
COMMENT ON FUNCTION admin_delete_user(integer, boolean, boolean) IS 'Exclui usuário do sistema. Opcionalmente remove autenticação (auth.users) e transações. Sempre remove histórico de chat do WhatsApp/N8N baseado no celular.';
...
CREATE POLICY "dependentes_veem_contas_com_permissao" ON contas_bancarias
    FOR SELECT USING (
        usuario_id IN (
            SELECT u.auth_user
            FROM usuarios u
...
CREATE POLICY "dependentes_veem_contas_tipo_conta_permitido" ON contas_bancarias
    FOR SELECT USING (
        usuario_id IN (
            SELECT u.auth_user
            FROM usuarios u
            ...
            WHERE d.auth_user_id = auth.uid()
              AND d.status = 'ativo'
              AND (d.permissoes->'tipos_conta_permitidos') @> to_jsonb(contas_bancarias.tipo_conta)
              AND (
                  (d.permissoes->>'pode_gerenciar_contas')::boolean = true
                  OR (d.permissoes->>'pode_ver_dados_admin')::boolean = true
              )
...
CREATE POLICY "Usuarios podem criar suas proprias contas" ON contas_bancarias
...
CREATE POLICY "Usuarios podem atualizar suas proprias contas" ON contas_bancarias
    FOR UPDATE USING (
        usuario_id = auth.uid()
        OR usuario_id IN (
            SELECT u.auth_user
...
CREATE POLICY "Usuarios podem excluir suas proprias contas" ON contas_bancarias
    FOR DELETE USING (
        usuario_id = auth.uid()
        OR usuario_id IN (
            SELECT u.auth_user
...
CREATE POLICY "dependentes_veem_cartoes_com_permissao" ON cartoes_credito
    FOR SELECT USING (
        usuario_id IN (
            SELECT u.auth_user
            FROM usuarios u
...
CREATE POLICY "dependentes_veem_cartoes_tipo_conta_permitido" ON cartoes_credito
    FOR SELECT USING (
        usuario_id IN (
            SELECT u.auth_user
            FROM usuarios u
            ...
            WHERE d.auth_user_id = auth.uid()
              AND d.status = 'ativo'
              AND (d.permissoes->'tipos_conta_permitidos') @> to_jsonb(cartoes_credito.tipo_conta)
              AND (
                  (d.permissoes->>'pode_gerenciar_cartoes')::boolean = true
                  OR (d.permissoes->>'pode_ver_dados_admin')::boolean = true
              )
...
CREATE POLICY "Usuários podem criar seus próprios cartões" ON cartoes_credito
    FOR INSERT WITH CHECK (
        usuario_id = auth.uid()
        OR usuario_id IN (
            SELECT u.auth_user
...
CREATE POLICY "Usuários podem atualizar seus próprios cartões" ON cartoes_credito
    FOR UPDATE USING (
        usuario_id = auth.uid()
        OR usuario_id IN (
            SELECT u.auth_user
...
CREATE POLICY "Usuários podem deletar seus próprios cartões" ON cartoes_credito
    FOR DELETE USING (
        usuario_id = auth.uid()
        OR usuario_id IN (
            SELECT u.auth_user
...
DROP POLICY IF EXISTS "Usuarios podem inserir ativos" ON investment_assets;
CREATE POLICY "Usuarios podem inserir ativos" ON investment_assets
FOR INSERT
TO authenticated
WITH CHECK (true);
...
USING (
  usuario_id IN (
    ...
    WHERE d.auth_user_id = auth.uid()
      AND d.status = 'ativo'
      AND (
        (d.permissoes->'tipos_conta_permitidos') @> to_jsonb(contas_bancarias.tipo_conta)
        AND (
          ((d.permissoes->>'pode_gerenciar_contas')::boolean = true)
          OR ((d.permissoes->>'pode_ver_dados_admin')::boolean = true)
        )
...
USING (
  usuario_id IN (
    ...
    WHERE d.auth_user_id = auth.uid()
      AND d.status = 'ativo'
      AND (
        (d.permissoes->'tipos_conta_permitidos') @> to_jsonb(cartoes_credito.tipo_conta)
        AND (
          ((d.permissoes->>'pode_gerenciar_cartoes')::boolean = true)
          OR ((d.permissoes->>'pode_ver_dados_admin')::boolean = true)
        )
...
DROP FUNCTION IF EXISTS update_contas_bancarias_updated_at();
...
-- Remover policy permissiva que permite qualquer usuário criar ativos
DROP POLICY IF EXISTS "Usuários podem criar ativos" ON investment_assets;
...
BEGIN
  ...
  SELECT 
    ...
    u.nome as principal_nome,
    u.email as principal_email,
    u.plano as principal_plano,
...
COMMENT ON FUNCTION admin_list_dependentes IS 'Lista todos os usuários dependentes com filtros e paginação. Retorna dados do dependente + dados do usuário principal. Criada em 19/01/2026.';
...
COMMENT ON FUNCTION admin_update_dependente IS 'Atualiza dados de um usuário dependente. Admin pode atualizar qualquer dependente. Criada em 19/01/2026.';
...
COMMENT ON FUNCTION admin_delete_dependente IS 'Exclui um usuário dependente. Opcionalmente remove de auth.users. Sempre remove histórico de chat do WhatsApp/N8N. Criada em 19/01/2026.';
...
COMMENT ON FUNCTION admin_reset_dependente_password IS 'Reseta a senha de um usuário dependente que possui conta de login. Criada em 19/01/2026.';
...
-- Atualizar função link_existing_user_on_signup para suportar dias_free = 0
...
BEGIN
    ...
    ELSIF NOT config_restricao THEN
        ...
        BEGIN
            ...
            RAISE LOG 'Usuário criado com sucesso: nome=%, email=%, plano_id=%, dias_free=%', nome_usuario, NEW.email, plano_free_id, dias_free;
...
-- ✅ Função link_existing_user_on_signup atualizada
-- ✅ Suporta dias_free = 0 (data_final_plano = ontem)
-- ✅ Middleware já bloqueia via isBlocked = true
-- ✅ Sem impacto em usuários existentes
-- =====================================================
